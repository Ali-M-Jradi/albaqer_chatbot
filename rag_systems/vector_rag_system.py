...
# =====================================================
# QUERY EXPANSION (STEP 3 IMPROVEMENT)
# =====================================================
def expand_query(query: str) -> str:
    """
    Expand query with synonyms and related terms for better retrieval.

    Why? Users ask questions in different ways:
    - "Is this ruby real?" vs "synthetic ruby detection"
    - "4Cs" vs "diamond grading factors"

    Query expansion adds related terms so the search finds ALL relevant documents.
    """
    query_lower = query.lower()

    # Gemstone synonyms and related terms
    expansions = {
        # Agate varieties (EXPANDED)
        "agate": "agate chalcedony banded stone quartz microcrystalline",
        "carnelian": "carnelian red agate orange chalcedony",
        "onyx": "onyx black agate banded chalcedony",
        "sardonyx": "sardonyx brown agate banded",
        "moss agate": "moss agate dendritic green inclusion",
        "yemeni agate": "yemeni yemen agate carnelian",
        "banded": "banded layered striped fortification",
        "chalcedony": "chalcedony agate microcrystalline quartz",
        # Precious stones
        "ruby": "ruby corundum red gemstone",
        "sapphire": "sapphire corundum blue gemstone",
        "emerald": "emerald beryl green gemstone",
        "diamond": "diamond carbon brilliant",
        # Grading and quality
        "4cs": "4Cs cut color clarity carat diamond grading",
        "4c": "4Cs cut color clarity carat diamond grading",
        "synthetic": "synthetic lab-created lab-grown artificial man-made",
        "fake": "fake synthetic imitation simulant counterfeit",
        "real": "real natural genuine authentic",
        "valuable": "valuable price worth value quality premium",
        "care": "care maintenance cleaning storage protect",
        "identify": "identify identification detect authenticate test",
        "grading": "grading grade quality assessment evaluation",
        "treatment": "treatment enhancement heated irradiated filled",
        "gia": "GIA Gemological Institute America standard",
        "quality": "quality grade premium excellent superior",
        "price": "price value cost worth market",
        "origin": "origin source provenance location mine",
    }
...
def get_embeddings_model():
    ...
    UPGRADED MODEL: all-mpnet-base-v2
    - 768 dimensions (vs 384 in MiniLM)
    - Better semantic understanding
    - ~20% accuracy improvement
    - Still free and runs locally
    """
    return HuggingFaceEmbeddings(
        model_name="sentence-transformers/all-mpnet-base-v2",  # UPGRADED from all-MiniLM-L6-v2
...
# =====================================================
# STEP 3: SEMANTIC SEARCH FUNCTION (ENHANCED)
# =====================================================
def semantic_search(query: str, top_k: int = 5, filter_dict: Dict = None) -> List[Dict]:
    """
    Perform semantic search on knowledge base with query expansion

    Args:
        query: Search query
        top_k: Number of results to return (INCREASED from 3 to 5)
    ...
    try:
        ...
        # STEP 2: Perform similarity search with INCREASED depth (k=5)
        # Why k=5? Cast a wider net to find all potentially relevant documents
        # We can then select the best ones after scoring
        if filter_dict:
            results = vectorstore.similarity_search_with_score(
                query=expanded_query, k=top_k, filter=filter_dict
            )
        else:
            results = vectorstore.similarity_search_with_score(
                query=expanded_query, k=top_k
            )
...
def rag_query(question: str, context_filter: Dict = None) -> Dict[str, Any]:
    ...
    """
    # Retrieve relevant documents (INCREASED to top 5 for better context)
    results = semantic_search(question, top_k=5, filter_dict=context_filter)
